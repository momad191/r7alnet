...
<Fragment>
  ...
  <th class="text-right" style={{fontFamily:'Changa',color:'#fff',backgroundColor:'#424242',fontWeight:'bold',float:'',fontSize:'11px'}}>التصنيف</th>
  ...
  <tbody style={{borderBottom:'#000'}}>
   ...
   {/* <td>
     ...
     <Link to={`/editPost/${props.exercise._id}`} className='btn btn-primary fontnty-font'
 ...
 <td><p  style={{ fontFamily:'Changa' ,color:"#31B404" , fontSize:"13px", fontWeight:'bold'}}  > {props.exercise.name}</p></td>
 <td><p  style={{ fontFamily:'Changa' ,color:"#000" , fontSize:"13px", fontWeight:'bold'}}  > {props.exercise.Publish}</p></td>
 <td><p  style={{ fontFamily:'Changa' ,color:"#B40486" , fontSize:"13px", fontWeight:'bold'}} className='post-date '> {props.exercise.CategoryName}</p></td>
 <td>   <h5  ><Link to={`/posts/${props.exercise._id}`} style={{fontFamily:'Changa' , fontSize:"13px", fontWeight:'bold',color:'#0101DF'}}>  {props.exercise.title}  </Link></h5> </td>
 <td> <div class="portfolio-img"> <Link to={`/posts/${props.exercise._id}`}> <img style={{width:'120px',height:'80px'}} alt="" src={props.exercise.image}/></Link></div> </td>                   
...
handlePageClick = (e) => {
  ...
  exerciseList() {
    ...
    .filter(post=>{
      return post.CategoryName.toLowerCase().indexOf(this.state.search.toLowerCase())>=0
    })
    .map(currentexercise => {
  ...
  render() {
    return (
      <div>
        ...
        >  البحث في المنشورات   <i class="fa fa-search"></i> </h1>

      

      
             <div class="col-lg-12 col-md-12 col-sm-12">
              <Link to="/searchPostsByTitle" title="Dashboard" className=''
              style={{padding:'5px',marginRight:'3px', fontFamily:'Changa',color:'#fff',backgroundColor:'#000',fontWeight:'bold',float:'',fontSize:'13px'}}
              ><span  
                >بالعنوان  </span> </Link>


<Link to="/searchPostsByCategory" title="Dashboard" className=''
          ...
          {/* <Link to="/searchPostsByDate" title="Dashboard" className=''
           style={{padding:'5px',marginRight:'3px', fontFamily:'Changa',color:'#fff',backgroundColor:'#000',fontWeight:'bold',float:'',fontSize:'13px'}}
          ><span  
          >بالتاريخ  </span> </Link> */}
...
<Link to="/searchPostsByStatus" title="Dashboard" className=''
           style={{padding:'5px',marginRight:'3px', fontFamily:'Changa',color:'#fff',backgroundColor:'#000',fontWeight:'bold',float:'',fontSize:'13px'}}
          ><span  
          >بحالة النشر  </span> </Link>


          

        <Link to="/searchPostsByUser" title="Dashboard" className=''
           style={{padding:'5px',marginRight:'3px', fontFamily:'Changa',color:'#fff',backgroundColor:'#000',fontWeight:'bold',float:'',fontSize:'13px'}}
          ><span  
          >بالمستخدم  </span> </Link>
              ...
              <span class="input-group-addon" id="basic-addon1">  <i className="fas fa-search"></i><label className="form-label">البحث </label></span>
              </div>
              </div>
